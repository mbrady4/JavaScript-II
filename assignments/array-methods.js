...
runners.map(runner => {
  let capitalizedName = runner.first_name.toUpperCase();
  firstNamesAllCaps.push(capitalizedName)
});
...
// console.log('Average Donation: ' + ticketPriceTotal/numRunners);

// Large Donor Companies (Those with donations > 200)
let largeDonors = runners.filter(runner => runner.donation > 200);
let largeDonorsCos = []
largeDonors.forEach(donor => largeDonorsCos.push(donor.company_name));
// console.log(largeDonorsCos);

// Sort Runners alphatically by last name
const sorted = runners.sort((a,b) => a.last_name.localeCompare(b.last_name));
console.log(sorted);